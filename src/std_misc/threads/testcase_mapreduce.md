...
Стандартната библиотека предоставя наготово чудесни първични типове за
многонишковост[^threading]. Тези типове, съчетани с правилата за
[единосъщност] и владеене на променливите, автоматично предотвратяват _гонките
за данни_[^data_races].

Правилата за _единосъщност на данните_[^aliasing] – само една менима (за писане) препратка
или (XOR – изключващо или) много неменими (за четене) препратки – автоматично предотвратяват
...
В този пример ще изчислим сумата от всички цифри в блок от числа. Ще направим
това, като разпределим парчетата от блока в различни нишки. Всяка нишка ще изчисли
сумата на нейния си подблок от цифри. После ще съберем получените междинни
суми, изчислени от всяка нишка.
...
fn main() {
    ...
    // Ще изчислим сбора от всички цифри чрез многонишков алгоритъм, наречен
...
16278424637452589860345374828574668";
    ...
    for (i, data_segment) in chunked_data.enumerate() {
        ...
        //
        // Ръждьо е достатъчно умен да отгатне '-> u32' от затварянето,
        // така че можеше даже да не го указваме изрично.
        //
        ...
        children.push(thread::spawn(move || -> u32 {
            ...
            let result = data_segment
                        ...
                        .chars()
                        // …преобразуваме текстовите знаци в числови стойности…
            ...
            // различни нишки.
            println!("обработихме участък {}, сума={}", i, result);

            // Не ни е нужен израз "return", защото Ръждьо е "ѝзразен език",
            // последно изчисленият израз във всеки блок става автоматично
            // стойност за връщане.
            result
...
* [затваряния][closures], семантика на [преместване на владението][move] и [`move` closures][move_closure]
