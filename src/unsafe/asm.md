...
Това ще вмъкне „NOP (no operation)” нареждане[^instruction] в асемблерния код,
създаден от компилатора. Забележете, че всички извиквания на `asm!` трябва да
са в `unsafe` блок, тъй като те могат да вмъкнат произволни нареждания към
машината и да счупят иначе непроменлив от програмиста код. Нарежданията за
...
четимостта и позволява преподреждане на нарежданията без да се променя реда на
аргументите.
...
гарантира, че ще сложи и двете в един и същи регистър.
