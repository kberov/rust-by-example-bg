...
Това може да бъде приличен първи опит на начинаещ за четене на редове от файл.
...
fn read_lines(filename: &str) -> Vec<String> {
    read_to_string(filename) 
        ...
        .lines() // Разделяме низа на низови отрязъци и ги слагаме в повторител.
        .map(String::from) // От всеки отрязък правим низ.
        .collect() // Събираме ги заедно във вектор.
}
```

Забележете, че и в двата примера се налага да превърнем препратката `&str`,
върната от `lines()`, във владимия тип `String` с помощта съответно на
`.to_string()` и `String::from`.
...
// Изходът е обгърнат в `Result`, за да разреши намиране на съвпадения при грешки.
// Връща повторител (`Iterator`) от `BufReader` за поредово четене във файла.
...
Този подход е по-сполучлив, отколкото да се създава в паметта обект от тип `String` с
