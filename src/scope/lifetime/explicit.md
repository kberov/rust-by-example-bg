...
fn failed_borrow<'a>() {
    ...
    // Грешка!: `_x` не живее достатъчно продължително
    let y: &'a i32 = &_x;
    // Опитът да се ползва живот `'a` като изрично отбелязване за тип вътре във
    // функцията дава грешка, защото животът на `&_x` е по-кратък от този на `y`. Кратък
    // живот не може насила да бъде приведен към по-продължителен.
}
...
fn main() {
    ...
    // казано, животът на `four` и `nine` трябва да е по-продължителен от този
    // на `print_refs`.
    
    failed_borrow();
    // `failed_borrow` не съдържа препратки, които да насилят `'a` да бъде по-продължителен
    // от живота на функцията, обаче `'a` е по-продължителен. Понеже животът не е ограничаван
...
[Обобщения][generics] и [Затваряния][closures]
