...
владее памет в *купа*[^heap]. Ръждьо спазва принципа ПДИ (Придобиването на
Данни[^resource] е Инициализация) – [RAII][raii] (Resource Acquisition Is
Initialization). Това означава, че винаги, когато някой обект излезе от обхват,
неговият метод за унищожаване се извиква и владените от него данни се
освобождават.

Това поведение предпазва от бъгове, причинени от *утечки на
данни*[^res_leaks]. Повече никога няма да ви се наложи да освобождавате памет
...
[`Drop`]. Унищожителят се извиква, когато парчето данни излезе от обхват. Не е
задължително този отличител да бъде осъществен за всеки тип, а само когато
вашият тип изисква да се направи нещо при унщожаване на обекта.
...
[^stack]: стълп, стек – stack. Остаряла форма на „стълб”, кула. Подредена памет
  в програмата за временно съхранение. Там съхраняваните единици са подредени
  условно казано отдолу нагоре и може да се вземат само отгоре – последната
  влязла стойност излиза първа. Източник: БТР, 935 – София 1996, Наука и
  Изкуство. [Вижте Стек в Уикипедия.](https://bg.wikipedia.org/wiki/Стек).

[^heap]: куп – heap. Другият вид памет на програмата – не подреден, различен
  от стълпа, който е подреден.

[^resource]: данни, източник – resource

[^res_leaks]: утечки на данни (в паметта) – resource (memory) leaks
