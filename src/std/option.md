...
* `None` – указва неуспех или липса на стойност, и
* `Some(стойност)` – разнородна структура, която обгръща дадената `стойност` от тип `T`.

```rust,editable,ignore,mdbook-runnable
// Целочислено деление, което не предизвиква `panic!`
fn checked_division(dividend: i32, divisor: i32) -> Option<i32> {
    if divisor == 0 {
        // Неуспехът е представен като разновидността `None`
        None
    } else {
...
fn try_division(dividend: i32, divisor: i32) {
    ...
    // съответствия, като при всеки друг броител
